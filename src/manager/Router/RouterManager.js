import * as RouterKey from './RouterConfig'
import * as RouteFunction from './RouterFunction'
import HttpUtils from '../../utils/HttpUtils'
import { Navigation } from 'react-native-navigation';
import { Linking, DeviceEventEmitter, } from "react-native";
import AppStatusManager from '../AppStatusManager'
import PermissionManager from "@peppas/toolkit/src/PermissionModule";
...
function runURIActionWithParams(uri,
    ...
    // show modal
    if (uri.indexOf(RouterKey.Action_SHOW_MODAL) === 0) {
        const { pageName, title } = genParams;
        RouteFunction.showModalScreen(pageName, title, genParams);
        canAction = true;
    }

    // dismiss modal
    if (uri.indexOf(RouterKey.ACTION_DISMISS_MODAL) === 0) {
        RouteFunction.dismissModal(mNavigator);
        canAction = true;
    }
